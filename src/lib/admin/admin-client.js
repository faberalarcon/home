...
document.getElementById('resetVisitorBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    showToast('Visitor count reset', 'success');
    await loadStats();
  } catch (e) {
...
async function toggleOgImage(filename) {
  ...
  try {
    if (isCurrentOg) {
      ...
      showToast('OG image set — refresh to apply', 'success');
    }
...
async function saveContent() {
  ...
  try {
    ...
    showToast('Content saved — refresh to publish', 'success');
  } catch {
...
async function uploadMemberPhoto(member, file) {
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || 'Upload failed');
    showToast(`${member} photo updated — refresh to publish`, 'success');
...
async function deleteMemberPhoto(member) {
  ...
  try {
    ...
    if (!res.ok) throw new Error('Remove failed');
    showToast('Photo removed — refresh to publish', 'success');
    await loadSiteConfig();
  } catch {
    showToast('Remove failed', 'error');
  }
...
document.getElementById('rebuildBtn').addEventListener('click', async () => {
  if (!confirm('Refresh runtime content now? This only refreshes runtime data.')) return;
  ...
  try {
    ...
    showToast(data.message || 'Content refreshed!', 'success');
    btn.disabled = false;
