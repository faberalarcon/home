...
<div class="flex items-center justify-between mb-6">
  ...
  <a href={adminPath('/milestones')} class="text-sm px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 transition">+ New milestone</a>
</div>
      ...
      {#each data.milestones as m (m.id)}
        <tr class="border-b border-slate-800/50 last:border-0 {data.editing?.id === m.id ? 'bg-slate-800/40' : 'hover:bg-slate-800/20'}">
          ...
          <td class="px-4 py-3 text-right space-x-3">
            ...
            <a href={`${adminPath('/milestones')}?edit=${m.id}`} class="text-slate-400 hover:text-white text-xs">Edit</a>
  ...
  {#each data.milestones as m (m.id)}
    <div class="bg-slate-900 border rounded-xl p-4 {data.editing?.id === m.id ? 'border-orange-700/60 bg-slate-800/40' : 'border-slate-800'}">
      ...
      <div class="flex flex-wrap gap-2">
        ...
        <a href={`${adminPath('/milestones')}?edit=${m.id}`} class="text-sm px-3 py-2 rounded-lg bg-slate-800 text-slate-200 hover:bg-slate-700">Edit</a>
...
<div class="bg-slate-900 border border-slate-800 rounded-xl p-6">
  ...
  <form method="POST" action="?/save" class="space-y-4">
    ...
    <div class="flex gap-3 pt-2">
      ...
      {#if data.editing}
        <a href={adminPath('/milestones')} class="px-4 py-2 rounded-lg bg-slate-800 text-slate-300 text-sm hover:bg-slate-700 transition">
          Cancel
        </a>
      {/if}
    </div>
