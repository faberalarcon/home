...
import { webrtcSignalingUrl, isPrintActive } from '$lib/stats/server/printer';
...
// flows browser <-> printer directly, so this only works on the home LAN/tailnet.
// Gated to active prints server-side so the signaling can't be driven when idle.
